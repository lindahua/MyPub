...
function parseCsv(contents: string): Imported[] {
  return parseCSV(contents).map((row, i) => {
    ...
    return { citation_key: row.citation_key || row.key || `import-${fingerprint(row).slice(0, 12)}`, title, type: type(row.type), authors: authors(row.authors || row.author),
      ...(date ? { publication_date: date } : {}), ...(venue ? { venue: { name: venue } } : {}),
...
function parseBibtex(contents: string): Imported[] {
  ...
  for (const match of contents.matchAll(entry)) {
    ...
    records.push({ citation_key: match[2]!.trim(), type: type(match[1]), title: fields.title.replace(/[{}]/g, ""), authors: authors(fields.author), ...(fields.journal || fields.booktitle ? { venue: { name: fields.journal ?? fields.booktitle! } } : {}), ...(fields.year ? { publication_date: fields.year } : {}), ...(fields.doi || fields.eprint ? { identifiers: { ...(fields.doi ? { doi: normalizeDoi(fields.doi) } : {}), ...(fields.eprint ? { arxiv: normalizeArxiv(fields.eprint) } : {}) } } : {}), ...(fields.url ? { urls: [fields.url] } : {}), ...(fields.keywords ? { tags: fields.keywords.split(/[,;]/).map((tag) => tag.trim()).filter(Boolean) } : {}), raw: fields });
  }
