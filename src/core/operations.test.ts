...
test("CSV and JSON imports support review update, rejection, and deferral", async () => {
  ...
  try {
    const catalog = new Catalog({ root }); await catalog.initialize(); await catalog.add({ citation_key: "known", type: "journal", title: "Known, Quoted", authors: [{ name: "Author" }], identifiers: { doi: "10.2/known" }, tags: ["old"] });
    const csv = join(root, "update.csv"); await writeFile(csv, 'title,authors,doi,tags\n"Known, Quoted",Author,10.2/known,"new; tested"\n', "utf8"); await importFile(catalog, csv);
    const update = (await listReviews(catalog, "pending"))[0]!; assert.equal(update.kind, "update"); await decideReview(catalog, update.id, "accepted"); assert.deepEqual((await catalog.get("known")).tags, ["new", "tested"]);
...
test("exports escape content and the SQLite index contains all records", async () => {
  ...
  try {
    const catalog = new Catalog({ root }); await catalog.initialize(); await catalog.add({ citation_key: "escape", type: "journal", title: "A {Structured} Title", authors: [{ name: "A, Author" }], venue: "Journal, One", dates: { issued: "2024" }, tags: ["one", "two"] });
