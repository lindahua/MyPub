...
test("catalog workflows preserve distinct publications and derive incoming relations", async () => {
  ...
  try {
    ...
    const preprint = await catalog.add({ citation_key: "Ada2025Example", type: "arxiv", status: "submitted", title: "An Example Paper", authors: [{ name: "Ada Lovelace" }], dates: { submitted: "2025-01-01" }, identifiers: { arxiv: "2501.00001v2" }, urls: [], tags: ["example"] });
    const conference = await catalog.add({ citation_key: "Ada2026Example", type: "conference", title: "An Example Paper, Revised", authors: [{ name: "Ada Lovelace" }], dates: { issued: "2026" }, identifiers: { doi: "https://doi.org/10.1000/EXAMPLE" }, urls: [], tags: [] });
...
test("partial Scholar snapshots preserve unknown counts and do not claim local omissions", async () => {
  ...
  try {
    const catalog = new Catalog({ root }); await catalog.initialize(); const publication = await catalog.add({ citation_key: "scholar2026", type: "journal", title: "Citation Paper", authors: [{ name: "A Person" }], dates: { issued: "2026" } }); const source = join(root, "scholar.csv");
    await writeFile(source, "title,year,citation_count,scholar_id,observed_at\nCitation Paper,2026,,record-1,2026-09-01T00:00:00Z\n", "utf8"); const result = await importScholarSnapshot(catalog, source, "partial"); assert.equal(result.matched[0]?.citation_count, undefined); assert.deepEqual(result.local_only, []); assert.deepEqual(result.matched[0]?.matched_publication_ids, [publication.id]);
  } finally { await rm(root, { recursive: true, force: true }); }
});
...
test("refresh imports do not erase unobserved fields or truncate curated authors", async () => {
  ...
  try {
    const catalog = new Catalog({ root }); await catalog.initialize(); await catalog.add({ citation_key: "complete2026", type: "journal", title: "Complete Record", authors: [{ name: "First Author" }, { name: "Second Author" }], dates: { issued: "2026" }, identifiers: { doi: "10.1000/complete" }, urls: ["https://example.test/paper"], tags: ["kept"] }); const source = join(root, "refresh.csv");
    await writeFile(source, "title,year,doi,authors\nComplete Record,2026,10.1000/complete,First Author\n", "utf8"); await importFile(catalog, source); const review = (await listReviews(catalog, "pending"))[0]!; assert.equal(review.changes.some((change) => change.field === "authors" || change.field === "urls" || change.field === "tags"), false);
  } finally { await rm(root, { recursive: true, force: true }); }
});
