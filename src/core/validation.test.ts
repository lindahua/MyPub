...
test("catalog rejects duplicate identifiers, keys, missing records, and invalid relations", async () => {
  ...
  try {
    const first = await catalog.add({ citation_key: "one", type: "journal", title: "One", authors: [{ name: "A" }], identifiers: { doi: "10.1/ONE" } });
    const second = await catalog.add({ citation_key: "two", type: "conference", title: "Two", authors: [{ name: "B" }] });
    await assert.rejects(catalog.add({ citation_key: "one", type: "other", title: "Duplicate", authors: [{ name: "C" }] }), errorCode("DUPLICATE_CITATION_KEY"));
    await assert.rejects(catalog.add({ citation_key: "doi", type: "other", title: "Duplicate DOI", authors: [{ name: "C" }], identifiers: { doi: "https://doi.org/10.1/one" } }), errorCode("DUPLICATE_IDENTIFIER"));
    await assert.rejects(catalog.get("missing"), errorCode("NOT_FOUND"));
    await assert.rejects(catalog.addRelation(first.id, first.id, "related_to"), errorCode("SELF_RELATION"));
    await catalog.addRelation(first.id, second.id, "related_to", "Peers");
    await assert.rejects(catalog.addRelation(first.id, second.id, "related_to"), errorCode("DUPLICATE_RELATION"));
...
test("filters, archival, and initialization are stable", async () => {
  const { root, catalog } = await fixture();
  try {
    const paper = await catalog.add({ citation_key: "filter", type: "workshop", status: "accepted", title: "Unicode Résumé Search", authors: [{ name: "Zoë" }], venue: "Test Conf", dates: { issued: "2025-03" }, tags: ["ML"] });
    assert.equal((await catalog.list({ query: "resume", year: 2025, venue: "test conf", type: "workshop", status: "accepted", tag: "ml" })).length, 1);
...
test("whole-catalog validation reports duplicate data, broken relations, cycles, and attachment state", async () => {
  ...
  try {
    ...
    const firstPath = join(catalog.publicationsDir, `${first.id}.json`); const secondPath = join(catalog.publicationsDir, `${second.id}.json`);
    ...
    assert.equal(result.valid, false); for (const code of ["DUPLICATE_CITATION_KEY", "DUPLICATE_RELATION", "BROKEN_RELATION", "RELATION_CYCLE", "ATTACHMENT_NOT_LOCAL"]) assert.equal(codes.has(code), true, code);
  } finally { await rm(root, { recursive: true, force: true }); }
});
