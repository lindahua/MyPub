...
test("arXiv XML is normalized while retaining DOI evidence", async () => {
  const original = globalThis.fetch;
  globalThis.fetch = async () => new Response(`<?xml version="1.0"?><feed xmlns:arxiv="http://arxiv.org/schemas/atom"><entry><title>  A Preprint Title </title><published>2026-02-03T00:00:00Z</published><author><name>Grace Hopper</name></author><author><name>Alan Turing</name></author><arxiv:doi>10.1007/PAPER</arxiv:doi></entry></feed>`, { status: 200 });
  try { const result = await lookupArxiv("https://arxiv.org/abs/2602.00001v3"); assert.equal(result.identifiers?.arxiv, "2602.00001"); assert.equal(result.identifiers?.doi, undefined); assert.equal(result.citation_key, "Hopper2026a"); assert.equal(result.authors.length, 2); }
  finally { globalThis.fetch = original; }
});
