...
export async function lookupArxiv(idInput: string, capture?: (payload: unknown) => void): Promise<AddPublicationInput> {
  ...
  return { citation_key: `arxiv_${arxiv.replace(/[^a-z0-9]/g, "_")}`, type: "arxiv", title: latest.title, authors: latest.authors.map(name => ({ name })), publication_date: versions[0]!.submission_date, submission_date: versions[0]!.submission_date, arxiv_versions: versions.map(({ version, title, authors, abstract, submission_date }) => ({ version, title, authors, abstract, submission_date })), identifiers: { arxiv }, venue: { name: "arXiv" }, urls: [`https://arxiv.org/abs/${arxiv}`], tags: [] };
}
