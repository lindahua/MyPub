...
test("CLI provides an end-to-end JSON and readable workflow", async () => {
  ...
  try {
    ...
    const scholar = join(root, "scholar.csv"); await writeFile(scholar, "scholar_id,title,year,citations\nentry,CLI Paper,2026,3\n"); const scholarImport = JSON.parse((await invoke(["--root", root, "--json", "gscholar", "import", scholar, "--partial"])).stdout); assert.equal(scholarImport.candidates.length, 1);
    const correctionFile = join(root, "scholar-correction.json"); await writeFile(correctionFile, JSON.stringify({ title: "Corrected CLI Paper" }));
    const correction = JSON.parse((await invoke(["--root", root, "--json", "gscholar", "correct", scholarImport.candidates[0].entry_id, "--json-file", correctionFile, "--reason", "Reviewed source typo"])).stdout); assert.equal(correction.state, "accepted");
    assert.equal(JSON.parse((await invoke(["--root", root, "--json", "gscholar", "correction", "release", scholarImport.candidates[0].entry_id, "title", "--reason", "Source fixed"])).stdout).state, "accepted");
