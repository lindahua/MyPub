...
test("CLI provides an end-to-end JSON and readable workflow", async () => {
  ...
  try {
    ...
    const secondInput = join(root, "second.json"); await writeFile(secondInput, JSON.stringify({ citation_key: "second2025", type: "arxiv", title: "Second Paper", authors: [{ name: "Second Author" }], identifiers: { arxiv: "2501.00002" }, publication_date: "2025-01-01", submission_date: "2025-01-01", arxiv_versions: [{ version: 1, submission_date: "2025-01-01", title: "Second Paper", authors: ["Second Author"], abstract: "Original abstract" }] }), "utf8"); const second = JSON.parse((await invoke(["--root", root, "--json", "add", "--json-file", secondInput])).stdout);
