...
test("CLI provides an end-to-end JSON and readable workflow", async () => {
  ...
  try {
    ...
    const input = join(root, "publication.json"); await writeFile(input, JSON.stringify({ citation_key: "cli2026", type: "conference", title: "CLI Paper", authors: [{ name: "Command Author" }], dates: { issued: "2026" }, tags: ["cli"] }), "utf8");
    ...
    const update = join(root, "update.json"); await writeFile(update, JSON.stringify({ venue: "CLI Conference" }), "utf8"); assert.equal(JSON.parse((await invoke(["--root", root, "--json", "update", "cli2026", "--json-file", update])).stdout).venue, "CLI Conference");
    ...
    const scholar = join(root, "scholar.csv"); await writeFile(scholar, "title,year,citations\nCLI Paper,2026,3\n", "utf8"); assert.equal(JSON.parse((await invoke(["--root", root, "--json", "scholar", "import", scholar, "--partial"])).stdout).matched.length, 1);
    ...
    assert.equal((await invoke(["--root", root, "--json", "validate", "--skip-attachments"])).code, 0); const indexed = JSON.parse((await invoke(["--root", root, "--json", "index", "rebuild"])).stdout); assert.equal(indexed.indexed, 2); assert.equal(JSON.parse((await invoke(["--root", root, "--json", "conflicts"])).stdout).length, 0); assert.equal(JSON.parse((await invoke(["--root", root, "--json", "status"])).stdout).git, true); assert.equal(JSON.parse((await invoke(["--root", root, "--json", "sync", "--message", "CLI fixture"])).stdout).conflicts.length, 0);
    const archived = JSON.parse((await invoke(["--root", root, "--json", "archive", "cli2026"])).stdout); assert.equal(archived.status, "archived"); await invoke(["--root", root, "--json", "archive", second.id]); assert.match((await invoke(["--root", root, "list"])).stdout, /No publications/);
