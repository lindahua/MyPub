...
export function formatAudit(r: AuditResult, details = false): string {
  const lines = [`Publications:    ${r.statistics.publications}`, `Scholar entries: ${r.statistics.scholar_entries}`, `Links:           ${r.statistics.links}`, `Errors:          ${r.errors}`, `Warnings:        ${r.warnings}`, `Acknowledged:    ${r.acknowledged_warnings}`];
  ...
  for (const f of r.findings) {
    if (f.acknowledged_by && !details) continue;
    lines.push("", `${f.severity.toUpperCase()} ${f.code}`, `  ${line(f.message)}`, ...f.paths.map(p => `  ${line(p)}`));
    if (details) { lines.push(`  Fingerprint: ${f.fingerprint}`); if (f.acknowledged_by) lines.push(`  Acknowledged by: ${f.acknowledged_by}`); if (f.field) lines.push(`  Field: ${line(f.field)}`); if (f.record_ids.length) lines.push(`  IDs: ${f.record_ids.join(", ")}`); if (f.values !== undefined) lines.push(`  Values: ${JSON.stringify(f.values)}`); }
  }
