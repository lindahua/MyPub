...
| `gscholar_entry_id` | optional UUID or array of at least two unique UUIDs | Confirmed Scholar associations. Omission means none; exactly one is stored as a scalar; two or more are stored as an ordered unique array. Each entry should belong to at most one publication; shared entries produce an audit-only error and remain admitted by write-time validation. Candidates never appear here. |
| `type` | required enum | One of `preprint`, `conference`, `workshop`, `journal`, `book-chapter`, `thesis`, `other`. It describes this record, not a related version. `preprint` covers arXiv and other preprint services; the provider is represented separately by identifiers and venue. |
...
The optional top-level `abstract` represents the current publication abstract. For arXiv, populate it from the latest retrieved version; retain every historical abstract in `arxiv_versions`. Existing records may omit it. Source refresh proposes changes for review; a source that omits an abstract does not erase an existing one. DOI imports convert supplied JATS/XML abstract markup to plain text. JSON/native interchange preserves the field, and CSV/BibTeX use an `abstract` field. The desktop viewer renders it as text in expanded publication details, never as executable HTML.

Every arXiv-backed preprint requires its base identifier and complete version history. `publication_date` and `submission_date` both equal the date of v1, from arXiv's original `published` timestamp. Each version's date comes from that version's `updated` timestamp. Later revisions never change the publication date/year. Revision dates must not decrease. Missing history or version metadata blocks admission; refresh must retrieve all versions before accepting an update, not silently save a partial history. The record's current title and ordered author credits describe the latest retrieved version; historical title, authors, and abstract remain in each immutable source-backed snapshot. Preserve established author identity links only when supported by the updated byline. Do not use a conference/journal date or DOI for its separate preprint record. Re-fetching may extend history; retain source evidence for every observation. Complete means through the latest version retrieved, not a claim that arXiv can never receive another revision. Non-arXiv preprints use the same `preprint` publication type but omit the arXiv identifier and `arxiv_versions`.
...
```json
{
  ...
  "reviewed_corrections": {
    "title": "74c6a5ce-d1b6-4367-a577-997c66d9d86b"
  },
  "source_review_id": "74c6a5ce-d1b6-4367-a577-997c66d9d86b",
  "citation_history": [
    {
...
| `title` | required non-empty string | Last observed source title, unless protected by an accepted local correction. |
...
| `pub_type` | optional enum | Reviewed classification of the Scholar entry: `journal`, `conference`, `preprint`, `workshop`, `patent`, `thesis`, `supp_material`, `unpublished`, `software`, `tech_report`, or `incomplete`. `patent` covers both granted patents and patent applications. `unpublished` identifies a rejected or submission-only paper retained by a service such as OpenReview, without an arXiv or formal publication version. `software` identifies a software or toolbox citation rather than a paper. `tech_report` identifies a technical report, including a challenge technical report associated with a workshop. `incomplete` identifies a malformed or fragmentary Scholar record whose displayed title or metadata does not constitute a usable publication entry; it is excluded from matching. Omission means the entry has not been classified confidently or is another kind. This field describes the Scholar entry itself and is not inferred from a linked publication record. |
| `venue` | optional non-empty string | Last observed source venue text, unless protected by an accepted local correction. |
| `year` | optional integer `1000`–`9999` | Last observed source publication year, or a protected reviewed correction. It determines the entry directory. |
| `publication_date` | optional non-empty string | Literal source publication date, including partial or malformed source text, unless protected by an accepted correction. It is not constrained to a curated local date and does not override `year` automatically. |
| `volume`, `issue`, `pages` | optional non-empty strings | Literal source bibliographic values, or protected reviewed corrections, independently of curated publication fields. |
...
| `presence` | required enum | `present` means last observed at `last_seen_at` without subsequent confirmed absence; `absent` requires applicable complete-capture evidence. An absent entry is retained for history, excluded from matching, and atomically unlinked from publications. |
| `absent_since` | conditionally required timestamp | Required only for `absent`; the complete capture time that first established absence. |
| `reviewed_corrections` | optional field-to-review map | Marks locally corrected Scholar fields and the accepted review authorizing each correction. Supported keys are `title`, `authors`, `venue`, `year`, `publication_date`, `volume`, `issue`, `pages`, `publisher`, `patent_office`, `application_number`, `description`, `scholar_url`, `cited_by_url`, and `authors_text`. The map is omitted when empty. |
...
No `field_observed_at` or other per-field time map is stored. `updated_at` records the latest accepted record change, including policy, correction, or citation changes. Retained metadata can come from different captures; original payloads remain in reviews without requiring separate timestamps on those fields. `first_seen_at`, `last_seen_at`, and `absent_since` describe record presence, not individual field freshness.

`reviewed_corrections` is provenance and refresh protection, not a second value store. The corrected value is stored directly in the named entry field; an optional source field can be removed by correction while its key remains in the map. Every mapped review UUID must identify an accepted review targeting this entry with a matching accepted proposal. The review retains the reason and points to the source review containing the unmodified observation. Later Scholar captures remain immutable evidence but do not replace protected fields. `mypub gscholar correction release` records a later accepted decision and removes protection for one field without guessing a replacement value.

`authors_completeness: "complete"` requires evidence that the retained ordered list is complete. `partial` means known truncation/omissions; `unknown` means completeness was not established. An empty parser result is normally `[]` with `unknown`, not a claim of an authorless work. Ellipses are preserved in `authors_text`/evidence, never inserted as author names. Update `authors` and its completeness together. A sparse capture may update `authors_text` while retaining a fuller accepted array and its completeness; raw overview text need not be identical to that array. Missing detail fields do not erase retained values or fill them from the curated publication. Scholar refresh does not derive, replace, or remove the locally reviewed `pub_type`, and does not replace fields listed in `reviewed_corrections`.
...
Confirmed matches live only in `publication.gscholar_entry_id`. Omit the field for no links, use a UUID scalar for one, and use an ordered array of at least two unique UUIDs for multiple Scholar entries representing the same bibliographic publication. Empty and one-element arrays are invalid. A candidate is a review proposal targeting that publication, using `operation: "link"`, `path: "/gscholar_entry_id"`, `candidate_ids` containing the entry UUID, and `proposed` containing the complete resulting scalar-or-array value. Accepting adds the candidate without replacing distinct confirmed links; optimistic validation prevents stale proposals from overwriting concurrent changes. A specific unlink removes only that entry and canonicalizes the remaining value; an unlink without an entry removes all associations.

For a publication with multiple Scholar entries, derive one component citation count per linked entry from its latest observation. The combined current count is the sum only when every component is numeric; if any latest component is `null`, the combined current count is `null`. Always label a numeric multi-entry sum as potentially overlapping because Scholar does not reveal whether the entries share citing papers.
...
An acknowledged audit warning is an accepted, evidence-only `kind: "change"` review with no proposals. Its evidence uses provider `mypub-audit`, parser version `mypub-audit/1`, and a payload whose `action` is `acknowledge_warning`. The payload retains the complete finding plus its `finding_fingerprint`; `input_fingerprint` contains the same lowercase SHA-256 value. `decision_note` records why no catalog correction is appropriate. Targets identify the principally affected records. The fingerprint covers the rule code, severity, aspect, sorted record UUIDs, field, and any compared values; it excludes presentation paths and wording. Therefore file renames do not invalidate a decision, while changed comparison values do. Audit errors cannot use this acknowledgement workflow. Git history supplies the deciding committer.
...
A terminal (`accepted` or `rejected`) historical `create` proposal is an audit snapshot, not a live entity. It may retain the former publication type value `arxiv`; validation interprets that value as `preprint` only while checking the snapshot. Live publication records and pending proposals must use `preprint`, so reopening such a proposal requires updating it before it can be accepted again.
