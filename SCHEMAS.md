...
| `gscholar_entry_id` | optional UUID or array of at least two unique UUIDs | Confirmed Scholar associations. Omission means none; exactly one is stored as a scalar; two or more are stored as an ordered unique array. Several publications may also point to the same entry. Candidates never appear here. |
