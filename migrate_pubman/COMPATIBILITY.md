...
- **Scalar Scholar links:** 410 confirmed matches can be inverted into `publication.gscholar_entry_id`; no publication has several linked entries. The other 56 publications remain unlinked. All 582 entries belong to one profile, and their external citation IDs have a consistent profile prefix. The mirror can retain all 172 unlinked entries, including 35 excluded entries.
...
- **Venues and publication kinds:** 33 conference, 16 journal, five workshop, and one preprint venue map to the venue catalog. The preprint venue is arXiv, with 127 publications; there is no observed non-arXiv preprint venue requiring a new publication type for this snapshot. The other publication counts are 296 conference, 38 journal, and five workshop.
...
No general preprint type, author URL field, alternate citation provider, per-field metadata timestamps, or publication status is required to accommodate this snapshot. The source's lost credited spellings/suffixes cannot be reconstructed by adding fields; preserve available evidence and leave unsupported assertions unresolved.

Version-2 implementation, conversion, and cutover remain future work. The seven decisions and date clarification are now documented and their snapshot implications checked. Remaining owner/destination/attachment inputs have not been guessed or treated as already confirmed.
