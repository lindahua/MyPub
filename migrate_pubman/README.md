...
The snapshot-based [compatibility review](COMPATIBILITY.md) includes the 7 September 2026 recheck against the revised authoritative SCHEMAS.md. The decisions below are agreed version-2 design requirements, not implemented formats; the review records the settled publication-date model and separates remaining migration inputs from nonblocking audit findings.
...
| Exclusion flag, reason, confirmer ID/time | Entry `matching: {policy: "excluded", reason, decision_review_id}` referencing an accepted migration review with the original actor ID/time; no confirmed links on excluded entries |
