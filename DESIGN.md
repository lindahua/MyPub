...
| Publication → Google Scholar entries (zero or more) | Publication's optional scalar-or-array `gscholar_entry_id` |
...
For journal papers, the curated `publication_date` is the journal issue publication date, preserving its known precision. Store online-first publication separately in `online_date`; it does not replace the issue date. An unknown issue date remains unknown until verified. Review source dates before accepting them as issue dates, and retain Scholar’s literal source year even when it differs from the curated issue year unless that value is itself demonstrably malformed and locally corrected through review. Thus the TPAMI paper “Temporal Segment Networks for Action Recognition in Videos” is counted in 2019 by issue publication, not in its 2018 online-first year.

Publications have no lifecycle `status` field or status filter. Catalog archiving is represented solely by optional `archived_at`; restore removes it. Archived publications cannot retain Scholar associations: archiving unlinks every entry in the same validated transaction, and restoration does not recreate them. Active in the catalog does not mean formally published. Metadata uses one `updated_at` per record, without per-field timestamps; source evidence remains in reviews, and citation observations keep their own dates.
...
An optional `reviewed_corrections` map identifies fields whose live values were corrected locally by accepted reviews. The corrected values remain directly readable on the entry; the map stores only the authorizing review UUIDs. Those reviews link back to immutable Scholar evidence containing the raw values and record the reason for correction. Refresh continues to capture raw observations in new evidence but does not overwrite protected fields. Use `mypub gscholar correct ENTRY --json-file PATCH --reason TEXT`; JSON `null` removes a malformed optional value. Use `mypub gscholar correction release ENTRY FIELD --reason TEXT` after the source is fixed so later refreshes may update that field again.

An entry may additionally store a locally reviewed `pub_type`: `journal`, `conference`, `preprint`, `workshop`, `patent`, `thesis`, `supp_material`, `unpublished`, `software`, `tech_report`, or `incomplete`. `patent` covers both granted patents and patent applications. `unpublished` identifies a rejected or submission-only paper retained by a service such as OpenReview, without an arXiv or formal publication version. `software` identifies a software or toolbox citation rather than a paper. `tech_report` identifies a technical report, including a challenge technical report associated with a workshop. `incomplete` identifies a malformed or fragmentary Scholar record whose displayed title or metadata does not constitute a usable publication entry; such entries are excluded from matching. The value classifies the Scholar entry from its own metadata or explicit review and must not be copied or inferred from a linked publication. Omit it when the entry is another kind or the available evidence is not sufficiently decisive. Scholar refresh preserves this field.
...
An unlinked Scholar entry is valid and remains in the mirror even if there is no corresponding local publication. Omitting `gscholar_entry_id` means a publication has no confirmed association. Store exactly one confirmed entry as a UUID scalar for backward compatibility and readability; store two or more confirmed entries as an ordered array of unique UUIDs. One-element and empty arrays are non-canonical and invalid. Candidate links remain in reviews. Linking another entry promotes a scalar to an array; unlinking collapses two remaining links back to a scalar and removes the field when none remain.

Both directions can be multiple: one Scholar entry may be explicitly linked from several equivalent local records, and one publication may link several Scholar entries when Scholar splits versions or duplicates of that same bibliographic publication. Matching must not use either capability to attach an arXiv Scholar entry to a conference or journal record, or vice versa. Section 6's type-correspondence rule selects the local bibliographic version represented by each Scholar entry. Derive associations from publication records rather than storing a second list. Shared and combined counts must be labeled; even the sum across distinct entries is not asserted to equal Scholar's profile total. Google describes how entries and overlapping citations can be merged. [Google Scholar duplicate and citation guidance](https://scholar.google.com/intl/en/scholar/citations.html)
...
Candidate matches live in link proposals targeting a publication's `gscholar_entry_id`. A proposal records the candidate entry UUID and the complete resulting scalar-or-array value. Accepting a match checks the expected old value and current revision, adds rather than replaces an existing distinct association, and preserves unique entry IDs. Never select an arbitrary publication when an identifier has multiple matches.

When the catalog contains both a preprint and a conference or journal publication for the same work, match the Scholar entry to the publication record whose bibliographic type the Scholar entry represents. An entry whose observed Scholar venue identifies an arXiv or other preprint links to the `preprint` record; an entry whose observed venue identifies conference proceedings links to the corresponding `conference` or `workshop` record; and a journal entry links to the `journal` record. A same or normalized-equal title is not sufficient to override this type correspondence. If the matching typed record is absent or the Scholar venue is too ambiguous to determine the represented type, leave the entry unlinked for review rather than attaching it to another version of the work. This rule selects the represented bibliographic version; publication relations continue to connect the separate local versions.
...
Use `mypub gscholar update` to fetch the selected profile directly, or import supplied exports and profile-table snapshots. The live update pages through the English-language profile, refreshes unprotected overview metadata and citation counts, and fetches detail pages for previously unknown entry IDs. It stages a complete capture before one recoverable catalog transaction. Requests have a 30-second timeout and a randomized 3–8 second interval; blocked requests, malformed pages, repeated IDs, ambiguous pagination, or failed new-entry details abort without catalog mutations. Page and detail progress goes to stderr. The final readable summary reports observed, added and refreshed entries, citation checks, newly missing, restored, and total missing entries. `--json` returns those counts plus reconciliation results on stdout. The command family is `gscholar` only; there is no `scholar` alias. The selected owner profile must already be configured. Google documents bibliographic export formats; do not assume those exports contain stable entry IDs or citation counts. Inspect and report available fields. Rows lacking an external ID remain in import reviews until matched to an existing entry or supplemented with an entry ID/URL; do not manufacture a Scholar identity from a title. [Google Scholar profile help](https://scholar.google.com/intl/en/scholar/citations.html)
...
Each citation observation contains its own `observed_at`, a non-negative `count` or `null`, and a source-review reference supporting its Google Scholar origin. An optional `estimated` flag records source-supported approximation/exactness; omission means that distinction was not recorded. The current count is derived from the latest observation, even if it is null; an older positive count must not silently replace a newer null. Append null when a successful citation check cannot find the count, including a complete capture newly confirming an entry's absence. Metadata-only imports and failed requests add no count sample. A publication without a confirmed Scholar link has a derived null count. For multiple linked entries, retain and display every component count and derive the combined count by summing their latest observations only when all are numeric; any current null makes the combined current count null. Label every multi-entry sum as potentially overlapping because Scholar does not establish distinct citing-paper sets. Do not fabricate an entry or deduplicate counts without source evidence. Reviewed corrections preserve prior evidence and history.
...
The reconciliation report distinguishes matched entries, eligible unmatched entries, excluded entries, rejected publication–entry pairs, and pending candidates. It also shows local publications without links, presence, metadata discrepancies, and shared-entry associations, with record update/capture times and citation observation times. An unlinked publication is not automatically proof that it is absent from Scholar. Display partial bylines and incomplete capture coverage clearly without claiming per-field freshness. User-invoked live scraping is implemented; scheduled scraping remains outside the current scope. A disappeared entry is marked `absent` only after a complete capture omits it. The retained Scholar record preserves its metadata and history, while the same accepted update excludes it from matching and atomically removes any publication links. Reappearance restores source presence but does not silently reverse the exclusion decision. Live captures preserve requested URLs and HTML in immutable review evidence (`mypub-scholar-web/1`); absent citation text remains null rather than an inferred zero. [Google Scholar access guidance](https://scholar.google.com/intl/en/scholar/help.html)
...
mypub gscholar link / unlink / exclude / include / correct / correction release
mypub review reopen
mypub audit
...
`mypub validate` enforces blocking format/reference constraints. `mypub audit` performs read-only, offline inspection directly from canonical JSON, including malformed records. [AUDITING.md](AUDITING.md) defines structural, completeness, linked-endpoint, and cardinality checks. Each Scholar entry should link to at most one publication; a publication may link multiple entries. Shared-entry and duplicate-arXiv errors are audit-only and do not prevent import, storage, export, migration, or synchronization of otherwise valid records. Output includes stable semantic finding fingerprints, findings, acknowledgements, coverage, and type-pair statistics, with `--details` and `--json` modes. `mypub audit acknowledge` is the explicit mutating subcommand: it stores the exact warning and rationale as an accepted review with immutable `mypub-audit` evidence under `catalog/reviews/`. Default audit output separates acknowledged warnings from actionable warnings; changed comparison values produce a different fingerprint and require a new decision. Audit errors cannot be acknowledged. Exit codes are 0 (warnings allowed), 4 (audit errors), 1 (operationally incomplete), and 2 (usage). No unique arXiv index is permitted. `gscholar exclude/include` and `review reopen` expose the decisions described in section 6. These commands are implemented in the current CLI.

`mypub history` exposes commit-based change history and record/decision attribution from section 2.2, including full commit ID, recorded committer name/email/time, and the relevant change. `mypub status` uses compact labeled rows for library, branch/upstream, the full current commit ID, local uncommitted file count, upload/download commit counts, and last sync. The human-readable last-sync row shows local date/time with its UTC offset and retains the stored timestamp in square brackets. Individual modifications are omitted by default. In an interactive terminal, status uses bold magenta for uncommitted changes, nonzero upload/download counts, and missing Git/upstream setup; bold red for Attention messages and file conflicts; and bold blue for the next action. Colors use the terminal’s theme-defined ANSI palette rather than fixed RGB values; bold adds emphasis independently of color. Routine clean/zero values stay plain. Redirected output, `--json`, terminals with `TERM=dumb`, and a nonempty `NO_COLOR` environment variable disable styling. With `--details`, an uncommitted-file list shows change status, record kind, and publication title or review summary where available. Conditional Attention and Next rows show problems and the immediate action; omit routine catalog/LFS health and explanatory paragraphs. It distinguishes saved local edits, commits waiting to upload/download, absent upstream configuration, detached branches, last successful sync, and synchronization conflicts. Uncommitted repository changes await commit (`dirty`); only local commits count as pending upload (`pending_upload`), including retained commits without an upstream. Remote comparison is explicitly based on the last fetched state, with no network request. `--json` preserves structured output, exposes the current commit as `commit`, and includes per-file changes (path, status, optional label and previous path). The core exposes the same derived history for later UI use without creating an application user database.
...
| A publication has no Scholar match, one match, or two confirmed entries representing versions of the same bibliographic publication | No link is omitted, one link is scalar, and two confirmed same-type links form a unique array with component and potentially-overlapping combined counts |
...
| A publication is saved, archived, and restored | No publication status field exists; archiving atomically removes all Scholar links and sets optional archived_at; restoring does not recreate those associations |
...
| Scholar splits versions or duplicates into separate entry identities | Reviewed linking may retain several same-publication entries, preserves each historical count series, and labels their combined sum as potentially overlapping |
