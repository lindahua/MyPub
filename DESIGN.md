...
`mypub validate` enforces blocking format/reference constraints. `mypub audit` performs read-only, offline inspection directly from canonical JSON, including malformed records. [AUDITING.md](AUDITING.md) defines structural, completeness, linked-endpoint, and cardinality checks. Each Scholar entry should link to at most one publication; a publication may link multiple entries. Shared-entry and duplicate-arXiv errors are audit-only and do not prevent import, storage, export, migration, or synchronization of otherwise valid records. Output includes findings, coverage, and type-pair statistics, with `--details` and `--json` modes. Exit codes are 0 (warnings allowed), 4 (audit errors), 1 (operationally incomplete), and 2 (usage). No unique arXiv index is permitted. `gscholar exclude/include` and `review reopen` expose the decisions described in section 6. These commands are implemented in the current CLI.
